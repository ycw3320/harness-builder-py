...
- [x] 1-D 여정 완결 — 생성 시점에 **용도**를 먼저 묻는 `ExportTargetDialog`(새 프로젝트 / 이미 있는
  프로젝트에 적용 = 기존 레포 원클릭, 결과를 그 자리에서 미리 알림) + 완료 다이얼로그에 [만든 파일 보기]
  (1-A 프리뷰 재사용)와 학습 루프 안내(확인→수정→`.harness.json` 공유) + 우패널 '생성 방식' 라벨을
  파일 구성이 아닌 용도 기준으로 통일
